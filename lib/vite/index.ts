...
function generateCode(
  ...
  if (sentry) {
    ...
    lines.push(
      ...
      "",
      "const _sentryIgnoredTransactions = [",
      "  /^GET \\/cdn-cgi\\/partyserver\\/set-name\\/?$/,",
      "  /^GET \\/connections\\/?$/,",
      "  /^GET \\/(?:\\.env|\\.git|adminer\\.php|info\\.php|phpinfo\\.php|public\\/phpinfo\\.php|backend\\/\\.env|wp-|xmlrpc\\.php|boaform\\/|cgi-bin\\/|actuator\\/|server-status)/,",
      "];",
      "",
      "function _sentryTransactionName(samplingContext: { name?: string; attributes?: Record<string, unknown> }): string {",
      "  if (typeof samplingContext.name === 'string') return samplingContext.name;",
      "  const attributes = samplingContext.attributes ?? {};",
      "  for (const key of ['sentry.name', 'http.route', 'http.target', 'url.path']) {",
      ...
      "}",
      "",
      "function _sentryTraceSampleRate(name: string, environment: string): number {",
      "  if (_sentryIgnoredTransactions.some((pattern) => pattern.test(name))) return 0;",
      "  const envName = environment.toLowerCase();",
      "  const isProd = envName === 'production' || envName === 'prod';",
      "  if (name.startsWith('invoke_agent ') || name.startsWith('agent.step ') || name.includes('/invoke')) {",
      "    return isProd ? 0.2 : 0.05;",
      "  }",
      "  if (name.startsWith('POST /agency/') || name.startsWith('POST /agents') || name.startsWith('POST /internal/register-agent')) {",
      "    return isProd ? 0.1 : 0.02;",
      "  }",
      "  if (name.startsWith('GET /agency/') || name.startsWith('GET /presence') || name === 'onStart' || name === 'onRequest') {",
      "    return isProd ? 0.02 : 0.005;",
      "  }",
      "  return isProd ? 0.02 : 0.001;",
      "}",
      "",
      "const _sentryOptions = (env: Record<string, unknown>) => {",
      "  const environment = (env.SENTRY_ENVIRONMENT || env.ENVIRONMENT || 'development') as string;",
      "  return {",
      "    dsn: env.SENTRY_DSN as string,",
      "    environment,",
      "    tracesSampler: (samplingContext: { name?: string; attributes?: Record<string, unknown> }) =>",
      "      _sentryTraceSampleRate(_sentryTransactionName(samplingContext), environment),",
      "    ignoreTransactions: _sentryIgnoredTransactions,",
