...
function generateCode(discovery, defaultModel, srcDir, outFile, sandbox, sentry = false) {
  ...
  if (sentry) {
    lines.push("");
    lines.push("const _sentryOptions = (env: Record<string, unknown>) => ({");
    lines.push("  dsn: env.SENTRY_DSN as string,");
    lines.push("  environment: (env.SENTRY_ENVIRONMENT as string) || 'production',");
    lines.push("  tracesSampleRate: 1.0,");
    lines.push("});");
    lines.push("");
    lines.push("export const HubAgent = Sentry.instrumentDurableObjectWithSentry(_sentryOptions, _HubAgent);");
    lines.push("export const Agency = Sentry.instrumentDurableObjectWithSentry(_sentryOptions, _Agency);");
  } else {
    lines.push("export const HubAgent = _HubAgent;");
    lines.push("export const Agency = _Agency;");
  }
  if (sentry) {
    lines.push("export default Sentry.withSentry(_sentryOptions, _handler);");
  } else {
    lines.push("export default _handler;");
  }
  lines.push("");
  return lines.join("\n");
}
